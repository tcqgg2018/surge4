...
if (!v4.primaryAddress && !v6.primaryAddress) {
  ...
  $httpClient.get(url, function (error, response, data) {
    ...
    $done({
      ...
      content:
        (v4.primaryAddress ? `𝐈𝐏𝐯𝟒 : ${v4.primaryAddress} \n` : '') +
        (v6.primaryAddress ? `𝐈𝐏𝐯𝟔 : ${v6.primaryAddress}\n` : '') +
        (v4.primaryRouter && wifi.ssid ? `𝐑𝐨𝐮𝐭𝐞𝐫 𝐈𝐏𝐯𝟒 : ${v4.primaryRouter}\n` : '') +
        (v6.primaryRouter && wifi.ssid ? `𝐑𝐨𝐮𝐭𝐞𝐫 𝐈𝐏𝐯𝟔 : ${v6.primaryRouter}\n` : '') +
        `节点 𝐈𝐏 : ${info.query}\n` +
        `节点 𝐈𝐒𝐏 : ${info.isp}\n` +
        `节点位置 : ${getFlagEmoji(info.countryCode)} | ${info.country} - ${info.city
        }`,
      icon: wifi.ssid ? 'wifi' : 'simcard',
      'icon-color': wifi.ssid ? '#89EFFF' : '#F9BF45',
    });
  });
}
